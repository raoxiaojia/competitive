...
int main()
{
	...
	for (i=0;i<nveh;i++)
	{
		...
		for (j=0;j<nrides;j++)
		{
			...
			{
				int rec_prob = 0;
	//			if (nvehfree+d < rinfo[j].data[4]) rec_prob = 3500;
	//			else rec_prob = 2;
				if (i < nveh1)
				{
					rec_prob = 10000.0*sqrt((double)rinfo[j].data[6]/(rinfo[j].data[6]+d));
					if (rinfo[j].data[6] < 30) rec_prob *= 0.5*sqr2(((double)i*40+nveh)/((double)nveh+nveh*40));
					else rec_prob *= 1;
				}
				else 
				{
